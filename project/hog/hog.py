...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    score_lst = [dice() for _ in range(num_rolls)]
    if 1 in score_lst:
        return 1
    else:
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    for x in range(1, 11):
        dice_x = dice
        score = make_averaged(roll_dice, samples_count)(x, dice_x)
        if score > max_score:
            max_score = score
            idx = x
    return idx
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    # END PROBLEM 12
    if sus_update(0, score, opponent_score, six_sided) >= sus_update(6, score, opponent_score, six_sided):
        return 0
    else:
        return 6
