...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        dis = 0
        target = self.place
        while target != None and not target.is_hive :
            if dis < self.lower_bound:
                target = target.entrance
                dis += 1
                continue
            if dis > self.upper_bound:
                return None
...
class HungryAnt(Ant):
    """HungryAnt will eat random bees, even those on other players' homes."""
    ...
    def action(self, gamestate):
        """A HungryAnt that is next to a Bee, will eat the Bee."""
        # BEGIN Problem 6
        if self.turns_to_chew == 0:
            if self.place.bees:            
                bee = random_bee(self.place.bees)
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        now_at = self.place.exit
        while now_at is not None:
            if now_at.ant is not None:
                print("DEBUG:","double", now_at.ant)
                Ant.double(now_at.ant)
                if now_at.ant.is_container and now_at.ant.ant_contained:
                    Ant.double(now_at.ant.ant_contained)
