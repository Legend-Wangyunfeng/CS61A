...
def balanced(m):
    ...
    left_part = left(m)
    right_part = right(m)
    left_length = length(left_part)
    right_length = length(right_part)
    if total_mass(end(left_part)) * left_length == total_mass(end(right_part)) * right_length:
        return balanced(end(left_part)) and balanced(end(right_part))
    else:
        return False
...
def max_path_sum(t):
    ...
    if not t:
        ...
        left_part = branches(t)[0]
        right_part = branches(t)[1]
        left_sum = max_path_sum(left_part)
        right_sum = max_path_sum(right_part)
